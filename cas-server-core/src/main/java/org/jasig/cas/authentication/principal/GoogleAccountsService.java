...
public class GoogleAccountsService extends AbstractWebApplicationService {
    ...
    private String constructSamlResponse() {
        ...
        if (this.alternateUserName == null) {
            userId = getPrincipal().getName();
        } else {
            final List<Object> attributeValues = getPrincipal().getAttributeValues(this.alternateUserName);

            if (attributeValues != null && !attributeValues.isEmpty()) {
                userId = attributeValues.get(0).toString();
            } else {
                userId = getPrincipal().getName();
            }
        }
